# CLI Components Usage Assessment — Self-Improving Prompt
...
### 1. Purpose
Run ALL usage functions in the cli/ directory WITH PROPER HOOKS, capture their raw outputs, assess reasonableness without regex/code matching, and generate a comprehensive markdown report in tmp/.
...
- **Generate Report**: Create tmp/CLI_USAGE_REPORT_[timestamp].md

### 3. What Makes Output "Reasonable"
- Evidence of actual CLI execution (not just imports)
- Appropriate output for CLI commands
- No unhandled exceptions (unless testing error cases)
- Presence of expected CLI behavioral indicators

### 4. Recommended Usage Pattern for AI Agents
CLI components should use direct implementation for maximum AI-friendliness:
```python
if __name__ == "__main__":
    print("=== CLI Component Usage ===")
    # Direct execution without flag parsing
    result = cli_function("test command")
    print(f"Output: {result}")
    assert validate_cli_output(result), "Invalid output"
    print("✅ CLI usage demonstration complete")
```
This avoids the complexity of argparse or flag handling for basic usage examples.
