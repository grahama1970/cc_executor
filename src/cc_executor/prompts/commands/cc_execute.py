#!/usr/bin/env python3
"""
CC Execute - Two-Stage Evaluation Pattern Implementation
...
def validate_stage1(output: str, exit_code: int) -> Tuple[bool, str]:
    ...
    error_patterns = [
        "traceback", "Traceback"  # These are definitive error indicators
    ]
    
    # These patterns need more context
    contextual_patterns = [
        ("error:", ["# Output:", "print(f\"Error:", "ValueError:", "TypeError:", "except"]),
        ("exception", ["raise", "except", "# "]),
        ("failed", ["assert", "test", "# "])
    ]
    
    output_lower = output.lower()
    
    # Check definitive error patterns
    for pattern in error_patterns:
        if pattern.lower() in output_lower:
            return False, f"Output contains error pattern: {pattern}"
    
    # Check contextual patterns
    for pattern, allowed_contexts in contextual_patterns:
        if pattern.lower() in output_lower:
            # Check if it's in an allowed context
            context_found = False
            for context in allowed_contexts:
                if context.lower() in output_lower:
                    context_found = True
                    break
