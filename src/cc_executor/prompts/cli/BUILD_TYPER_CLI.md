# Typer-Based CLI with Extensive Hooks — Self-Improving Prompt
...
- **Evolution History**:
  ...
  | :------ | :---------------------------------------------------- | :----- |
  | v1      | Initial implementation with comprehensive hook system | TBD    |

---
## 🏛️ ARCHITECT'S BRIEFING (Immutable)

### 1. Purpose
Create a comprehensive Typer-based CLI for CC Executor that:
- Provides intuitive command-line interface for all CC Executor operations
- Integrates extensively with the hook system for pre/post command processing
- Matches the quality and structure of llm_call's CLI
- Saves raw responses to prevent AI hallucination
- Uses Redis for session tracking and history

### 2. Core Principles & Constraints
- Must use Typer framework (already in dependencies)
- Every command must trigger appropriate hooks
- Raw output must be saved to tmp/responses/
- Must integrate with existing WebSocket infrastructure
- Must provide both simple and advanced usage patterns
- Entry point: cc-executor (defined in pyproject.toml)

### 3. API Contract & Dependencies
- **CLI Commands Required:**
  - `server`: Start/stop/status management
  - `run`: Execute commands through WebSocket
  - `test`: Run stress tests and assessments
  - `history`: View execution history from Redis
...
- **Dependencies:**
  - WebSocket server at ws://localhost:8003/ws/mcp
  - Redis for history and session tracking
  - Hook system in cc_executor.hooks
    ...
    def decorator(func):
        async def async_wrapper(*args, **kwargs):
            ...
            context = HookContext(
                ...
                args=args,
                kwargs=kwargs,
                metadata={"cli": True, "timestamp": datetime.now().isoformat()}
            )
            
            # Run pre-hooks
            pre_result = await hook_registry.run_hooks(f"cli.{command_name}.pre", context)
            if not pre_result.proceed:
                console.print(f"[red]Command blocked by pre-hook: {pre_result.message}[/red]")
                raise typer.Exit(1)
            
            # Apply any context modifications from hooks
            if pre_result.context:
                kwargs.update(pre_result.context.kwargs)
            ...
            try:
                ...
                # Run post-hooks with result
                context.metadata["result"] = result
                post_result = await hook_registry.run_hooks(f"cli.{command_name}.post", context)
                
                # Return potentially modified result
                return post_result.data or result
                
            except Exception as e:
                # Run error hooks
                context.metadata["error"] = str(e)
                error_result = await hook_registry.run_hooks(f"cli.{command_name}.error", context)
                if error_result.proceed:
                    raise  # Re-raise if hooks say to proceed
                else:
                    console.print(f"[yellow]Error handled by hook: {error_result.message}[/yellow]")
                    raise typer.Exit(1)
...
def server_start(
    ...
    if hooks:
        hook_paths = hooks.split(",")
        for hook_path in hook_paths:
            try:
                hook_registry.load_hook_file(Path(hook_path))
                console.print(f"[green]✓[/green] Loaded hooks from {hook_path}")
            except Exception as e:
                console.print(f"[red]✗[/red] Failed to load hooks from {hook_path}: {e}")
...
def hooks_list(
    ...
    hooks = hook_registry.list_hooks(pattern)
    ...
    for hook_info in hooks:
        table.add_row(
            hook_info["point"],
            str(hook_info["priority"]),
            hook_info["handler"].__name__,
            hook_info["handler"].__module__
        )
...
def hooks_run(
    ...
    console.print(f"[bold]Triggering hook: {hook_point}[/bold]\n")
    
    # Run the hook
    async def run_hook():
        return await hook_registry.run_hooks(hook_point, context)
    
    result = asyncio.run(run_hook())
    
    # Display result
    console.print(f"Proceed: {'[green]Yes[/green]' if result.proceed else '[red]No[/red]'}")
    if result.message:
        console.print(f"Message: {result.message}")
    if result.data:
        console.print("Data:")
        console.print_json(data=result.data)
    if result.context and result.context != context:
        console.print("Modified Context:")
...
def hooks_reload():
    ...
    console.print(f"[bold]Reloading hooks from: {hooks_dir}[/bold]\n")
    ...
    for hook_file in hooks_dir.glob("*.py"):
        ...
        try:
            hook_registry.load_hook_file(hook_file)
            console.print(f"[green]✓[/green] Loaded {hook_file.name}")
            loaded += 1
        except Exception as e:
            console.print(f"[red]✗[/red] Failed to load {hook_file.name}: {e}")
    
    console.print(f"\n[bold]Loaded {loaded} hook files[/bold]")
...
#### **Step 1: Basic CLI Structure with Hook Integration**
*   **Goal:** Create a working Typer CLI with all command groups and basic hook integration
*   **Action:** Implement the CLI structure with server, test, history, hooks, and config command groups, all decorated with hook support
*   **Verification Command:** `python3 cli/BUILD_TYPER_CLI.md --help && python3 cli/BUILD_TYPER_CLI.md version`
*   **Expected Output:** Help text showing all command groups and version output
...
#### **Step 2: Server Commands with Process Management**
*   **Goal:** Implement server start/stop/status commands with proper process group management and hook integration
*   **Action:** Add server management commands that handle background processes correctly and trigger appropriate hooks
*   **Verification Command:** `python3 cli/BUILD_TYPER_CLI.md server start --help && python3 cli/BUILD_TYPER_CLI.md server status`
*   **Expected Output:** Help text for server start command and status showing server not running
...
#### **Step 3: WebSocket Command Execution**
*   **Goal:** Implement the run command with WebSocket communication and output streaming
*   **Action:** Add run command that connects to WebSocket server, streams output, and saves raw responses
*   **Verification Command:** `python3 cli/BUILD_TYPER_CLI.md run "echo test" --no-save --server ws://localhost:8003/ws/mcp`
*   **Expected Output:** Error about server not running or successful echo output
...
#### **Step 4: Test Commands with Assessment Integration**
*   **Goal:** Implement stress test and assessment commands that integrate with existing test infrastructure
*   **Action:** Add test commands that run stress tests and usage function assessments with hook support
*   **Verification Command:** `python3 cli/BUILD_TYPER_CLI.md test assess --help && python3 cli/BUILD_TYPER_CLI.md test stress --help`
*   **Expected Output:** Help text for both assessment and stress test commands
...
#### **Step 5: History and Metrics from Redis**
*   **Goal:** Implement history commands that read execution data from Redis
*   **Action:** Add history list and metrics commands with Redis integration
*   **Verification Command:** `python3 cli/BUILD_TYPER_CLI.md history list --limit 5 && python3 cli/BUILD_TYPER_CLI.md history metrics --days 1`
*   **Expected Output:** Empty history or Redis connection error

**--- EXECUTION LOG (Step 5) ---**
```text
<!-- Paste verification output here -->
```

#### **Step 6: Hook Management Commands**
*   **Goal:** Implement hook introspection and management commands
*   **Action:** Add commands to list, run, and reload hooks with detailed output
*   **Verification Command:** `python3 cli/BUILD_TYPER_CLI.md hooks list && python3 cli/BUILD_TYPER_CLI.md hooks reload`
*   **Expected Output:** List of registered hooks or message about loading hooks

**--- EXECUTION LOG (Step 6) ---**
```text
<!-- Paste verification output here -->
```

#### **Step 7: Configuration and Initialization**
*   **Goal:** Implement config management and environment initialization
*   **Action:** Add config show/validate commands and init command for environment setup
*   **Verification Command:** `python3 cli/BUILD_TYPER_CLI.md config show && python3 cli/BUILD_TYPER_CLI.md init --help`
*   **Expected Output:** Configuration table and init command help

**--- EXECUTION LOG (Step 7) ---**
```text
<!-- Paste verification output here -->
```

#### **Step 8: Usage Function with Response Saving**
*   **Goal:** Implement comprehensive usage function that demonstrates all CLI features and saves output
*   **Action:** Complete the usage function that tests all command groups and saves raw responses
*   **Verification Command:** `cd cli && python3 BUILD_TYPER_CLI.md && ls tmp/responses/`
*   **Expected Output:** Usage example output and saved response files

**--- EXECUTION LOG (Step 8) ---**
```text
<!-- Paste verification output here -->
```
...
*   **Test Script Path:** Run the usage function to verify all commands work
*   **Execution:** `python3 cli/BUILD_TYPER_CLI.md`

### 2. Self-Verification
*   **PREDICTION:** The usage function will test all major command groups, showing that each command's help is accessible and the CLI structure is complete. It will save its output to tmp/responses/ to prevent AI hallucination.
*   **Assertion:** The usage function contains assertions that verify all expected commands are present and working.

---
## 🔍 DEBUGGING BEST PRACTICES

This prompt follows all required debugging patterns:
1. Uses subprocess with proper process group management
2. Implements async stream draining for WebSocket communication
3. Saves all raw responses to prevent hallucination
4. Integrates extensively with the hook system
5. Uses Redis for persistent state management