# Hooks Usage Assessment — Self-Improving Prompt
...
### 1. Purpose
Execute ALL hook usage functions WITH FULL HOOK INTEGRATION, capture raw outputs, validate with Redis, and generate report in tmp/ using proper pre/post hooks.

### 2. Core Assessment Strategy
- **PRE-HOOKS**: setup_environment.py, check_task_dependencies.py, analyze_task_complexity.py
- **Execute with --test**: Run `python hook.py --test` with proper environment
- **POST-HOOKS**: truncate_logs.py, record_execution_metrics.py, claude_response_validator.py
- **Report Generation**: Create tmp/HOOKS_USAGE_REPORT_[timestamp].md

### 3. What Makes Hook Output "Reasonable"
- Demonstrates the hook's actual functionality
- Shows clear test scenarios with results
- Produces expected side effects (Redis keys, env changes)
- Contains success/failure indicators
- Logs are properly truncated when needed

### 4. Note on Hook Testing Patterns
Hooks use `--test` flags for safety (to avoid running in production), but for future AI-friendly components:
- Prefer direct `if __name__ == "__main__":` implementation
- This reduces cognitive load and execution complexity
- Hooks are an exception due to their production nature
...
#### Step 1: Run Assessment with Full Hook Chain
```bash
cd /path/to/cc_executor
source .venv/bin/activate
export PYTHONPATH="${PWD}/src:${PYTHONPATH}"
cd src/cc_executor/hooks
python3 ASSESS_ALL_HOOKS_USAGE.md
```

#### Step 2: View Generated Report
```bash
# Report is in local tmp/ directory
ls -la tmp/HOOKS_USAGE_REPORT_*.md
cat tmp/HOOKS_USAGE_REPORT_*.md
```

#### Step 3: Check Hook Chain Evidence
```bash
# See what was created in temp directory
ls -la tmp/hook_assess_*/

# Check Redis for hook evidence
redis-cli keys "assess:*"
...
This assessment graduates when:
1. Full pre/post hook chain executes successfully
2. All tests run with proper environment setup
3. Reports show hook chain evidence
4. Achieves 10:1 success/failure ratio

---
## 🔍 DEBUGGING PATTERNS

### Hook Chain Issues:
1. Verify all hooks are importable
2. Check Redis is running for metrics
3. Ensure temp directory is writable
4. Look for pre/post hook evidence in report

### Missing Hook Evidence:
```bash
# Debug why hooks aren't running
python3 -c "from cc_executor.hooks import setup_environment; print('Hook imports work')"

# Check individual hook
python3 setup_environment.py --test
```