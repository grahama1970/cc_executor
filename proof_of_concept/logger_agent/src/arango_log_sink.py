...
Run this script directly to test functionality:
  python arango_log_sink.py          # Runs working_usage() - stable, known to work
  python arango_log_sink.py debug    # Runs debug_function() - for testing new ideas
...
def _test_sink_usage():
    ...
    async def run_test():
        ...
        try:
            ...
            # Log with extra data
            with logger.contextualize(execution_id=execution_id, tags=["test", "sink"]):
                logger.info("Test log 1")
            with logger.contextualize(execution_id=execution_id, tags=["test"]):
                logger.warning("Test warning")
            ...
            logger.info(f"Logs found in isolated DB for {execution_id}: {stats['successful_writes']}")
...
# Additional test functions with logger.contextualize() usage
def test_high_volume_logging():
    """Test with high volume of logs."""
    for i in range(100):
        with logger.contextualize(test_id=f"volume_{i}"):
            logger.debug(f"High volume test {i}")

def test_large_messages():
    """Test with large log messages."""
    large_data = {"data": "x" * 10000}
    with logger.contextualize(extra_data=large_data):
...
def test_error_recovery():
    ...
    for i in range(10):
        with logger.contextualize(test="failover"):
            logger.error(f"Failover test {i}")

# Error handler - alternative approach to avoid logger.bind
def _handle_batch_error(self, batch, error):
    """Handle errors during batch processing."""
    logger.opt(exception=True).error(
        f"Batch write failed: {error}. Saving {len(batch)} logs for retry."
    )
