...
Run this script directly to test functionality:
  python agent_log_manager.py          # Runs working_usage() - stable, known to work
  python agent_log_manager.py debug    # Runs debug_function() - for testing new ideas
...
class AgentLogManager:
    ...
    def __init__(self):
        if not self._initialized:
            ...
            self.sink = None  # Add reference to ArangoLogSink
    ...
    def set_sink(self, sink) -> None:
        """Set the ArangoLogSink instance for context management."""
        self.sink = sink
    ...
    async def script_execution(self, script_name: str, metadata: Optional[Dict] = None):
        ...
        Yields a Loguru logger instance with `execution_id` and `script_name` contextualized.
        """
        execution_id = self.generate_execution_id(script_name)
        self.current_execution_id = execution_id
        
        # Set execution context on sink if available
        if self.sink:
            self.sink.set_execution_context(execution_id, script_name)
        
        # Start script run
        await self.start_run(script_name, execution_id, metadata)
        
        try:
            # Use contextualize as a context manager (fixes deprecation warning)
            with logger.contextualize(execution_id=execution_id, script_name=script_name):
                yield logger
            ...
            self.current_execution_id = None
            # Clear execution context on sink
            if self.sink:
                self.sink.set_execution_context(None, None)
    ...
    async def log_agent_learning(
        ...
        try:
            # Fix: Use asyncio.to_thread for synchronous insert operation
            await asyncio.to_thread(
                self.db.collection("agent_learnings").insert,
                doc
            )
...
async def working_usage():
    ...
    # Note: The yield from script_execution is now the contextualized logger
...
if __name__ == "__main__":
    ...
    async def main():
        ...
        try:
            ...
            # Get the manager and initialize it with test database
            manager = AgentLogManager()  # Create instance directly
            ...
            # Override the global instance with our test instance
            global _manager_instance
