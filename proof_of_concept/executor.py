...
# Import ProcessManager for consistent subprocess handling
try:
    from cc_executor.core.process_manager import ProcessManager
except ImportError:
    logger.warning("Could not import ProcessManager, will use basic subprocess")
    ProcessManager = None

# Configure logger to save to file for debugging
log_dir = Path(__file__).parent / "logs"
log_dir.mkdir(exist_ok=True)
logger.add(
    log_dir / "cc_executor_{time}.log",
    rotation="100 MB",
    retention="7 days",
    level="DEBUG",
...
Execute the analysis and return ONLY the timeout number."""
        ...
        try:
            ...
            timeout_result = await cc_execute(
                ...
                return_json=True  # Get structured response
            )
...
"""
        ...
        if return_json and isinstance(full_output, str) and 'json' in full_output.lower():
...
async def cc_execute_list(
    ...
    if sequential:
        ...
        for i, task in enumerate(tasks):
            ...
            result = await cc_execute(task, config, return_json=return_json, amend_prompt=amend_prompt)
        ...
        tasks_coroutines = [
            cc_execute(task, config, return_json=return_json, amend_prompt=amend_prompt) for task in tasks
        ]
