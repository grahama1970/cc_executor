...
model = "vertex_ai/gemini-2.0-flash-exp"  # Testing Vertex AI
query = """Write a haiku about Python programming"""
output_path = "/tmp/litellm_vertex_test.md"
...
fallback_models = ["gpt-3.5-turbo", "perplexity/sonar"]  # Fallback if Vertex fails
...
def try_completion(model_name: str, messages: List[Dict], **kwargs) -> Optional[Any]:
    ...
    try:
        ...
        if response and response.choices and response.choices[0].message.content:
            ...
            print(f"WARNING: Empty response from {model_name}", file=sys.stderr)
            return None
